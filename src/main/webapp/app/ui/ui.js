...
var ui = function() {
	...
	return {
		...
		loadEnviromment() {
			...
			var properties = [];
			if(ui.envirommentFile){
				var allProperties = ui.envirommentFile["properties"];

				for (var i = 0; i < allProperties.length; i++) {
					if(allProperties[i].elementType && allProperties[i].elementType.includes(type)){
		...
		handleException: function(error = "", statusError) {
			try {
				...
				if(statusError){
					ui.alert("Failed to execute action: " + statusError);
				}else{
					ui.alert(e);
				}
			}
...
$('#modal-button-load-enviromment').click(function() {
	'use strict';
	var fileInputEnv = $('#input-enviromment');
	if (!fileInputEnv.val()) {
		ui.alert('You must select all a input file to load', 'No Model file selected');
		$('#modal-load-enviromment').modal('hide');
		return;
	}

	try {
		ui.getFileInput(fileInputEnv[0], function(result) {
            ui.envirommentFile = JSON.parse(result);
			var logoName = "piStar " + ui.envirommentFile["enviromment"];
			var allButtons = ui.envirommentFile["buttons"];

			ui.generateTempButtons(allButtons);
